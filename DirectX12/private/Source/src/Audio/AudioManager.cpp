...
HRESULT AudioManager::loadOGGFile(const std::string& filePath, SoundData& outputData, int poolSize)
{
    ...
    //XAUDIO2_BUFFERを作成
    XAUDIO2_BUFFER buffer = {};
	buffer.AudioBytes = static_cast<UINT32>(totalBytes); // 波形データの合計バイト数
    buffer.pAudioData = outputData.audioData.data(); 
...
HRESULT AudioManager::loadMP3File(const std::string& filePath, SoundData& outputData, int poolSize)
{
    ...
    WAVEFORMATEX waveFormat = {};
    waveFormat.wFormatTag = WAVE_FORMAT_PCM;
    waveFormat.nChannels = static_cast<WORD>(info.channels);
    waveFormat.nSamplesPerSec = static_cast<DWORD>(info.hz);
    ...
    //XAUDIO2_BUFFERを作成
    XAUDIO2_BUFFER buffer = {};
    buffer.AudioBytes = static_cast<UINT32>(outputData.audioData.size());
    buffer.pAudioData = outputData.audioData.data();
