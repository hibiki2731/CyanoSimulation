	...
	mGame(game)
{
	...
	init_info.CommandQueue = mGraphic.getGraphicsCommandQueue();
...
void GUIDebugger::end()
{
	...
	ImGui::Render();
	mGraphic.getGraphicsCommandList()->SetDescriptorHeaps(1, mSrvHeap.GetAddressOf());
	ImGui_ImplDX12_RenderDrawData(ImGui::GetDrawData(), mGraphic.getGraphicsCommandList());
}
...
//
//	//インデックスが範囲内か判定
//	const int maxIdx = static_cast<int>(mCyanoSimulator->mIndivisual_headPointIdx.size()) - 1;
	...
	if (ImGui::Button("apply")) {
		...
		if (j[sceneName].contains(newName) && j[sceneName][newName].size() != 0) {
			...
			for (int i = 0; i < objectNames.size(); i++) {
				//初期オブジェクトに含まれる場合
				if (objectNames[i] == obj->mName) {
					objectNames[i] = newName;
