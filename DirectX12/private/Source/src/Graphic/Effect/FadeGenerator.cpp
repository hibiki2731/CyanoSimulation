	...
	isFadingOut(false)
{
	//フェード用パイプラインステート
	{
...

		{
			...
			hr = mGraphic.getDevice()->CreateGraphicsPipelineState(&pipelineDesc, IID_PPV_ARGS(PipelineStateFade.GetAddressOf()));
			assert(SUCCEEDED(hr));
		}
	} {}
...
void FadeGenerator::renderFade()
{
	if (isFadingIn || isFadingOut) {
		mGraphic.getGraphicsCommandList()->SetPipelineState(PipelineStateFade.Get());
		mGraphic.getGraphicsCommandList()->SetGraphicsRootSignature(RootSignatureFade.Get());
		mGraphic.getGraphicsCommandList()->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);

		//ルートパラメータにフェードのアルファ値をセット
		mGraphic.getGraphicsCommandList()->SetGraphicsRoot32BitConstants(0, 1, &currentFadeAlpha, 0);
		mGraphic.getGraphicsCommandList()->DrawInstanced(3, 1, 0, 0);
	}
}
