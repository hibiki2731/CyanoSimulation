...
void AssetManager::createMeshData(const std::string& meshID, const MeshFileData& meshFileData)
{
	...
	for (auto partsJson : json["parts"]) {
		...
		//頂点バッファ
		{
			...
			int numVertices = static_cast<int>(positions.size()) / 3;
			...
			//頂点バッファをつくる
			VertexBufferDescription desc = {static_cast<UINT>(numVertices), static_cast<UINT>(NumElementsPerMeshVertex)};
...
{
	return static_cast<UINT>(spriteVertices.size());
}

UINT AssetManager::getSpriteIndicesSize() {
	return static_cast<UINT>(spriteIndices.size());
}
...
void AssetManager::createSpriteBuffers()
{
	{
		//頂点バッファの作成
		VertexBufferDescription desc = { static_cast<UINT>(spriteVertices.size()) / 4, 4};
