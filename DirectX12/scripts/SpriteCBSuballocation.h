...
//2D用バッファ
struct SpriteCBSuballocationData {
	XMMATRIX world;
...
class SpriteCBSuballocation : public IConstantBufferSuballocation
{
public:
	SpriteCBSuballocation(const AlignedSizeInBytes& sizeInBytes, const ConstantBufferAddress& address, std::vector<void*>&& buffersOnCPU, std::vector<D3D12_GPU_VIRTUAL_ADDRESS>&& virtualAddressOnGPU);
...
private:
	SpriteCBSuballocationData mData;
