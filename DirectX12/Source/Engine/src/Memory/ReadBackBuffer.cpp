...
ReadBackBuffer::ReadBackBuffer(ID3D12Device& device, int sizeOfElement, int numElement)
...
void ReadBackBuffer::copyData(const void* resource, const size_t resourceSize)
{
	memcpy(mBuffersOnCPU, resource, resourceSize);
}

ID3D12Resource* ReadBackBuffer::getBufferOnGPU() const
{
	return mBuffersOnGPU.Get();
}

void* ReadBackBuffer::getBufferOnCPU() const
{
	return mBuffersOnCPU;
}

D3D12_RESOURCE_DESC ReadBackBuffer::getResourceDesc()
{
	D3D12_RESOURCE_DESC desc = CD3DX12_RESOURCE_DESC::Buffer(mSizeOfElement * mNumElement);
	return desc;
}

D3D12_HEAP_PROPERTIES ReadBackBuffer::getHeapProperties()
{
	...
	prop.MemoryPoolPreference = D3D12_MEMORY_POOL_UNKNOWN;			//メモリの配置先をL0プールに指定
